USE sqlDevToolboxAB
GO
IF OBJECT_ID('dbo.WNGrams2012_2B', 'IF') is not null DROP FUNCTION dbo.WNGrams2012_2B;
GO
CREATE FUNCTION dbo.WNGrams2012_2B(@string varchar(max), @N bigint)
/*****************************************************************************************
Purpose:
 WNGrams2012_2B accepts a varchar(max) input string (@string) and splits 
...
--===== Autonomous
 SELECT 
   ng.tokenNumber,
   ng.token
 FROM dbo.WNGrams2012_2B(@string,@N) ng;
...
--===== Against another table using APPLY
 ...
 CROSS APPLY dbo.WNGrams2012_2B(@string,@N) ng;
-----------------------------------------------------------------------------------------
Usage Examples:
...
--===== Example #1: Word-level Unigrams:
  SELECT
    ...
    ng.itemIndex,
    ng.item
  FROM dbo.WNGrams2012_2B('One two three four words', 1) ng;
...
--===== Example #2: Word-level Bi-grams:
  SELECT
    ...
    ng.itemIndex,
    ng.item
  FROM dbo.WNGrams2012_2B('One two three four words', 2) ng;
...
Programmer Notes:
 ...
 2. Kudo's to Eirikur Eiriksson; this function could not have been developed without 
    ...
    https://goo.gl/Gtru6A. The code may look substantially different but, WNGrams2012_2B
    is a hacked mutation of Eiriksson's DelimitedSplit8K_LEAD (see: https://goo.gl/f58x9t)

 3. Is faster than WNGrams2012_2B, the pre-2012 version, WNGrams_8K, which cannot leverage
    LEAD and which means a less efficient self-join against a derived set is required.

 4. WNGrams2012_2B uses spaces (char(32)) as the delimiter; the text must be pre-formatted
