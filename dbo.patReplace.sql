IF OBJECT_ID('dbo.patReplace', 'IF') IS NOT NULL DROP FUNCTION dbo.patReplace;
GO
CREATE FUNCTION dbo.patReplace
(
  @string  varchar(max),
  @pattern varchar(50),
  @replace varchar(20)
...
--===== Basic Syntax Example
 SELECT pr.NewString
 FROM dbo.patReplace(@String,@Pattern,@Replace);

--===== Replace numeric characters with a "*"
 SELECT pr.NewString
 FROM dbo.patReplace('My phone number is 555-2211','[0-9]','*') pr;
...
--==== Using againsts a table
 ...
 FROM @table t
 CROSS APPLY dbo.patReplace(t.oldstring,'[0-9]','*') pr;
